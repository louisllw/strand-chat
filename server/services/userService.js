...
export const checkUsernameAvailability = async (userId, username) => {
    ...
    if (username) {
      ...
      if (normalizedUsername !== current.username) {
        ...
        if (now.getTime() - lastUpdated.getTime() < cooldownMs) {
          throw new ServiceError(429, 'Username can only be changed every 7 days');
        }
...
export const listEmojiRecents = async (userId, limit) => {
  return getEmojiRecents(userId, limit);
};

export const saveEmojiRecent = async (userId, emoji) => {
